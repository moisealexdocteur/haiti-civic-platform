 ...
 * MIT licensed. Vendored locally to keep citizen tracking data off third-party services.
 */
(function () {
    'use strict';
    var factories = {"QRMode": function (require, module, exports) {
module.exports = {
    MODE_NUMBER :       1 << 0,
    MODE_ALPHA_NUM :    1 << 1,
    MODE_8BIT_BYTE :    1 << 2,
...
module.exports = QRCode;
    ...
    function requireModule(name) {
        ...
        if (!factories[name]) throw new Error('Unknown QR module: ' + name);
        var module = {exports: {}};
        cache[name] = module;
        factories[name](requireModule, module, module.exports);
        return module.exports;
    }

    function renderQr(element, value) {
        var QRCode = requireModule('index');
        var levels = requireModule('QRErrorCorrectLevel');
        var qr = new QRCode(0, levels.M);
        qr.addData(value);
        qr.make();
        var count = qr.getModuleCount();
        var quiet = 4;
        var size = count + quiet * 2;
        var parts = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + size + ' ' + size + '" role="img" aria-hidden="true" shape-rendering="crispEdges">', '<rect width="100%" height="100%" fill="#fff"/>', '<path fill="#101828" d="'];
        for (var row = 0; row < count; row++) {
            for (var column = 0; column < count; column++) {
                if (qr.isDark(row, column)) parts.push('M' + (column + quiet) + ' ' + (row + quiet) + 'h1v1h-1z');
            }
        }
        parts.push('"/></svg>');
